...
public class DBWrapper {
	...
	public void putWebpage(Webpage webpage){
		webpage.content = null;			// ===>>> this is dangerous but to save space!!!!
	...
	public boolean containsRobots(String hostName){
		...
		boolean res = accessor.robotIndex.contains(hostName);
		//System.out.println("db has this robotinfo == >> " + res);
		return res;
	}
	...
	public void setRobotLastVisitTime(String hostName){
		RobotInfo robotInfo = accessor.robotIndex.get(hostName);
		robotInfo.lastVisitTime = new Date().getTime();
		accessor.robotIndex.put(robotInfo);
	}
	...
	public boolean visitedURLContains(String url){
		synchronized (accessor.visitedURLIndex) {
			return accessor.visitedURLIndex.contains(url);
		}
	}
	
	public long getLastVisited(String url){
		synchronized (accessor.visitedURLIndex) {
			return accessor.visitedURLIndex.get(url).lastVisited;
		}
	}
	
	public void updateLastVisited(String url, long lastVisited){
		synchronized (accessor.visitedURLIndex) {
			accessor.visitedURLIndex.get(url).setLastVisited(lastVisited);;
		}
