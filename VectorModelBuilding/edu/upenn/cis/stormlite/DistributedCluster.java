	...
	private void createSpoutInstances(Topology topo, Config config) throws ClassNotFoundException{
		...
		for (String key: topo.getSpouts().keySet()) {
			...
			for (int i = 0; i < spout.getRight(); i++)
				try {
					IRichSpout newSpout = (IRichSpout)Class.forName(spout.getLeft()).newInstance(); // instantiate spout;
					...
					System.out.println("Error on instantiating spout: " + spout.getLeft().toString());
					e.printStackTrace();
				}
				catch (IllegalAccessException e) {
					System.out.println("Error on accessing spout: " + spout.getLeft().toString());
					e.printStackTrace();
				}

			System.out.println("Finish creating spout instances...");
		}
	}
		...
		for (String key: topo.getBolts().keySet()) {
			...
			for (int i = 0; i < localExecutors; i++)
				...
				try {
					...
					System.out.println("Error on instantiating bolt: " + bolt.getLeft().toString());
					e.printStackTrace();
				}
				catch (IllegalAccessException e) {
					System.out.println("Error on accessing bolt: " + bolt.getLeft().toString());
					e.printStackTrace();
				}
			System.out.println("Finish creating bolt instances...");
		}
	}
	...
	private void createRoutes(Topology topo, Config config) {
		...
		for (String stream: topo.getBolts().keySet()) {
			...
			if (topo.getBolts().containsKey(decl.getStream())) {
				...
				for (IRichSpout spout: spoutStreams.get(decl.getStream())) { // TODO check this;
		...
		System.out.println("Finish creating route...");
	}
