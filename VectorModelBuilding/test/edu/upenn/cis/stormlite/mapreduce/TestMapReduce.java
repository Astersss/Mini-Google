...
//		config.put("reduceclass", "test.edu.upenn.cis.stormlite.mapreduce.GroupWords");
        ...
        config.put("reduceexecutors", "8");
        config.put("numofworkers", "2");
			...
			try {
				...
				for (String dest: workers){
					...
					if (sendJob(dest, "POST", config, "definejob", 
							mapper.writerWithDefaultPrettyPrinter().writeValueAsString(job)).getResponseCode() != HttpURLConnection.HTTP_OK) {
